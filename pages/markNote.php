<?php
	...
	setcookie("myNoteUsername", 'nic', time()+2592000);
	...
	function show_error_exit($output,$show_return=true){
		...
		echo '<!DOCTYPE html><head><meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
			<style>body{font-family: "文泉驛正黑","Microsoft yahei UI","Microsoft yahei","微软雅黑","Lato",Helvetica,Arial,sans-serif !important;}button{border: 0;background: #3498DB;color: #fff;font-size: 16px;padding:5px 10px;box-shadow: 0 1px 3px rgba(100, 100, 100, 0.3);}</style><title>在线笔记</title></head>';
	...
	<body>
		...
		<div id="header">
			...
			<div class="header-title"  onclick="$('#force-home-form').submit();" >
				<h1 title="首页" style="display:inline-block;font-size:24px;color:#FCFCFC;border:0;padding:0;cursor:pointer;margin-top:-3px;">在线笔记</h1>
			</div>
		</div>
	...
	<body>
		...
		<?php if( $page_type == 'text_note' || $page_type == 'md_note' ) : ?>
			...
			<div id="header">
				...
				<!-- MarkNote标题 && 返回主页按钮 -->
				<div class="header-title"  onclick="window.location.href= 'http://wlbx.qlu.edu.cn'" >
					<h1 title="首页" style="display:inline-block;font-size:24px;color:#FCFCFC;border:0;padding:0;cursor:pointer;margin-top:-3px;">在线笔记</h1>
				</div>

				<!-- 登陆按钮 或 我的记事本按钮 -->
				<?php if(isset($username)) : ?>
					<button title="点击以打开记事本列表侧边栏，再次点击可关闭" class="header-btn" title="" style="float:left;" onclick="mynote_display();" ><div data-icon="ei-user"></div>所有记事本</button>
				<?php else : ?>
		...
		<?php if ( $page_type == 'home' ) : ?>
			...
			<div id="header">
				...
				<div class="header-title">
					<h1 title="首页" style="display:inline-block;font-size:24px;color:#FCFCFC;border:0;padding:0;cursor:pointer;margin-top:-3px;" onclick="$('#force-home-form').submit();" >在线笔记</h1>
				</div>
